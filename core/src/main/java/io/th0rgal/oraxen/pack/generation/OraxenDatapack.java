...
public abstract class OraxenDatapack {
    ...
    protected boolean isFirstInstall() {
        return Bukkit.getDatapackManager().getPacks().stream()
                .noneMatch(d -> d.getName().equals(this.name));
    }

    protected boolean isDatapackEnabled() {
        for (final Datapack dataPack : Bukkit.getDatapackManager().getPacks()) {
            if (!dataPack.getName().equals(this.name)) continue;
            return true;
        }
        return false;
    }
    ...
    protected void enableDatapack(final boolean enabled) {
        if (VersionUtil.isPaperServer()) {
            Bukkit.getDatapackManager().getPacks().stream()
                    .filter(datapack -> datapack.getName().equals(this.name))
                    .findFirst()
                    .ifPresent(datapack -> datapack.setEnabled(true));
        }
    }
