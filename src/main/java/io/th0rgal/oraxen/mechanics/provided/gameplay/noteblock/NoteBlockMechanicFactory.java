...
public class NoteBlockMechanicFactory extends MechanicFactory {
    ...
    public NoteBlockMechanicFactory(final ConfigurationSection section) {
        super(section);
        instance = this;
        ...
        OraxenPlugin.get().getResourcePack().addModifiers(getMechanicID(), packFolder ->
                ResourcePack.writeStringToVirtual(
    ...
    public static String getInstrumentName(final int id) {
    ...
    public static JsonObject getModelJson(final String modelName) {
    ...
    public static JsonObject getDirectionalModelJson(final String modelName, final NoteBlockMechanic mechanic, final NoteBlockMechanic parentMechanic) {
        final String itemId = mechanic.getItemID();
        final JsonObject content = new JsonObject();
        final DirectionalBlock parent = parentMechanic.getDirectional();
        final String subBlockModel = mechanic.getDirectional().getDirectionalModel(mechanic);
    ...
    public static String getBlockstateVariantName(final String instrument, final int note, final boolean powered) {
        return "instrument=" + instrument + ",note=" + note + ",powered=" + powered;
    }

    public static NoteBlockMechanic getBlockMechanic(final int customVariation) {
     ...
     */
    public static void setBlockModel(final Block block, final String itemId) {
        final NoteBlockMechanic mechanic = OraxenBlocks.getNoteBlockMechanic(itemId);
    ...
    private String getBlockstateContent() {
        final JsonObject noteblock = new JsonObject();
        noteblock.add("variants", variants);
        return noteblock.toString();
    }

    @Override
    public Mechanic parse(final ConfigurationSection itemMechanicConfiguration) {
        final NoteBlockMechanic mechanic = new NoteBlockMechanic(this, itemMechanicConfiguration);
        ...
        final DirectionalBlock directional = mechanic.getDirectional();
        ...
        if (mechanic.isDirectional() && !directional.isParentBlock()) {
            final NoteBlockMechanic parentMechanic = directional.getParentMechanic();
    ...
    @Override
    public NoteBlockMechanic getMechanic(final String itemID) {
        return (NoteBlockMechanic) super.getMechanic(itemID);
    }

    @Override
    public NoteBlockMechanic getMechanic(final ItemStack itemStack) {
     ...
     */
    public NoteBlock createNoteBlockData(final String itemID) {
        /* We have 16 instruments with 25 notes. All of those blocks can be powered.
         * That's: 16*25*2 = 800 variations. The first 25 variations of PIANO (not powered)
         * will be reserved for the vanilla behavior. We still have 800-25 = 775 variations
         */
...
//        if (farmblockList.isEmpty()) return;
        ...
        final BukkitTask task = farmBlockTask.runTaskTimer(OraxenPlugin.get(), 0, farmBlockCheckDelay);
