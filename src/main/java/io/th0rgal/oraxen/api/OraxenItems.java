...
public class OraxenItems {
            ...
            if (Settings.DEBUG.toBool()) {
                ...
                try {
                    if (VersionUtil.atOrAbove("1.21")) {
                        final JukeboxPlayableComponent component = new ItemStack(Material.MUSIC_DISC_CREATOR).getItemMeta()
                                .getJukeboxPlayable();
    ...
    private static void ensureComponentDataHandled() {
        if (VersionUtil.atOrAbove("1.21"))
            for (final Entry<File, Map<String, ItemBuilder>> entry : map.entrySet()) {
                final Map<String, ItemBuilder> subMap = entry.getValue();
                for (final Entry<String, ItemBuilder> subEntry : subMap.entrySet()) {
                    final String itemId = subEntry.getKey();
                    final ItemBuilder itemBuilder = subEntry.getValue();
                    if (itemBuilder == null)
                        continue;
                    final FoodComponent foodComponent = itemBuilder.getFoodComponent();
                    if (foodComponent == null)
                        continue;

                    final ConfigurationSection section = OraxenYaml.loadConfiguration(entry.getKey())
                            .getConfigurationSection(itemId + ".Components.food.replacement");
                    final ItemStack replacementItem = parseFoodComponentReplacement(section);
    ...
    @Nullable
    private static ItemStack parseFoodComponentReplacement(@Nullable final ConfigurationSection section) {
        if (section == null)
            return null;

        final ItemStack replacementItem;
        if (section.isString("minecraft_type")) {
            final Material material = Material.getMaterial(Objects.requireNonNull(section.getString("minecraft_type")));
            if (material == null) {
    ...
    public static ItemBuilder getBuilderByItem(final ItemStack item) {
    ...
    public static boolean hasMechanic(final String itemID, final String mechanicID) {
    ...
    public static String[] getItemNames() {
        return items.stream().filter(item -> {
            final ItemBuilder builder = OraxenItems.getItemById(item);
